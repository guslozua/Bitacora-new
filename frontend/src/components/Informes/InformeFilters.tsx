...
const InformeFilters: React.FC<InformeFiltersProps> = ({ tipo, onFilter, onExport }) => {
  ...
  const [conIncidentes, setConIncidentes] = useState<boolean>(false);
  ...
  const aplicarFiltros = () => {
    ...
    if (tipo === 'incidentes') {
      ...
      if (conIncidentes) filtros.conIncidentes = conIncidentes;
      if (orderBy) {
        filtros.orderBy = orderBy;
        filtros.orderDir = orderDir;
      }
            ...
            {tipo === 'guardias' && (
              <Col md={3}>
                <Form.Group className="mt-4">
                  <Form.Check
                    type="checkbox"
                    label="Solo con incidentes"
                    checked={conIncidentes}
                    onChange={(e) => setConIncidentes(e.target.checked)}
                  />
                </Form.Group>
              </Col>
            )}
