// models/codigo.model.js - VERSIÓN CON CÁLCULO DE TARIFAS
...
const Codigo = {
  ...
  findAll: async (options = {}) => {
    try {
      ...
      query += whereClause + ' ORDER BY codigo ASC';
  ...
  // ✨ FUNCIÓN MEJORADA PARA ENCONTRAR CÓDIGOS APLICABLES (CON SOPORTE MEDIANOCHE)
  findApplicable: async (fecha, horaInicio, horaFin) => {
    try {
      console.log('🔍 Buscando códigos aplicables:', { fecha, horaInicio, horaFin });
      ...
      console.log('📅 Día de la semana:', dayOfWeek, 'Es feriado:', esFeriado);
      
      // Obtener todos los códigos activos y vigentes
      const query = `
      ...
      console.log(`📋 Se encontraron ${rows.length} códigos candidatos`);
      ...
      const codigosAplicables = rows.filter(codigo => {
        ...
        if (!codigo.hora_inicio || !codigo.hora_fin) {
          console.log(`✅ Código ${codigo.codigo}: Aplica todo el día`);
          return true;
        }
        ...
        console.log(`${hayInterseccion ? '✅' : '❌'} Código ${codigo.codigo}: Horario ${codigo.hora_inicio}-${codigo.hora_fin} ${hayInterseccion ? 'intersecta' : 'NO intersecta'} con ${horaInicio}-${horaFin}`);
        
        return hayInterseccion;
      });
      
      console.log(`🎯 RESULTADO: ${codigosAplicables.length} códigos aplicables`);
  ...
  calcularImporteCodigo: async (codigo, incidente, tarifa) => {
    try {
      console.log('💰 Calculando importe para código:', codigo.codigo);
      ...
      switch (codigo.tipo_calculo) {
        case 'guardia_pasiva':
          ...
          detalle = `Guardia pasiva: $${tarifa.valor_guardia_pasiva}`;
          break;
          
        case 'hora_activa':
          // Guardia activa: valor por hora con fraccionamiento
          importe = tarifa.valor_hora_activa * duracionHoras;
          detalle = `${duracionHoras}h × $${tarifa.valor_hora_activa} = $${importe}`;
          break;
        ...
        case 'adicional_nocturno':
          ...
          detalle = `${horasNocturnas}h nocturnas × $${tarifa.valor_hora_activa} × ${factorNocturno} = $${importe}`;
          break;
  ...
  create: async (data) => {
    try {
      const { 
        ...
        // ✨ NUEVOS CAMPOS
        tipo_calculo, usa_tarifa_base, factor_adicional, unidad_facturacion
      } = data;
      ...
      const [existingCodigos] = await pool.query(
        'SELECT id, codigo FROM codigos_facturacion WHERE codigo = ?',
        [codigo]
      );
      
      if (existingCodigos.length > 0) {
        const error = new Error(`El código "${codigo}" ya existe en el sistema.`);
      ...
      const insertParams = [
        ...
        usa_tarifa_base !== undefined ? usa_tarifa_base : true,
  ...
  update: async (id, values) => {
    try {
      ...
      // Verificar conflictos de código
      if (codigo) {
        const [currentCodigo] = await pool.query(
          'SELECT codigo FROM codigos_facturacion WHERE id = ?',
          [id]
        );
        
        if (currentCodigo.length > 0 && codigo !== currentCodigo[0].codigo) {
          const [existingCodigos] = await pool.query(
            'SELECT id, codigo FROM codigos_facturacion WHERE codigo = ? AND id != ?',
            [codigo, id]
          );
